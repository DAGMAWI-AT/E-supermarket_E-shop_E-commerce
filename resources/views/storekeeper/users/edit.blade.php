...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('SKusers.update', $user->id)}}">
        @csrf
        @method('PATCH')
        ...
        <div class="form-group">
            ...
            <select name="role" class="form-control">
                ...
                @foreach($roles as $role)
                    ...
                    <option value="{{$role->role}}" {{(($role->role == 'employee') ? 'selected' : '')}}>Employee</option>
                    <option value="{{$role->role}}" {{(($role->role == 'storekeeper') ? 'selected' : '')}}>Storekeeper</option>
                    @endforeach
            </select>
          @error('role')
          <span class="text-danger">{{$message}}</span>
