    ...
    <table class="table message-table" id="message-dataTable">
      ...
      <tbody>
        ...
        <tr class="@if($message->read_at) border-left-success @else bg-light border-left-warning @endif">
          ...
          <td>
            <a href="{{route('Emessage.show', $message->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="View" data-placement="bottom"><i class="fas fa-eye"></i></a>
            <form method="POST" action="{{route('Emessage.destroy', [$message->id])}}">
              @csrf
              @method('delete')
